...
Tyto platformy často fungují ve~spolupráci oficiální systém v~\EVthree{}, ale jsou i~platformy s~vlastním operačním systémem. 
...
% Následně se aplikace spustí ve virtuálním stroji.

Celý systém s~vývojářskými nástroji a~dokumentací je volně k~dispozici na~webu \lego{}~\cite{legoMindstorms_download}. Díky tomu, že~\lego{} uvolnilo zdrojové kódy, mohly vzniknout alternativní systémy pro~\EVthree{} jako~\evThreeDev{} (kapitola~\ref{lego-ev3dev}) nebo \evRT{} (kapitola~\ref{lego-EV3RT}). 
...
Nalezneme zde i~správa připojení, v~rámci které lze vyhledávat a~připojovat se k dostupným \brick{\it ům}. 
...
V systému jde nastavit automatické připojování na konkrétní WiFi a~pak se již k~\brick{\it u} připojovat bezdrátově.
...
Nelze na něm přímo provozovat \textit{roscore}, kvůli nedostatečnému výkonu \brick{\it u}. 
...
Komunikace mezi PC obsahující \textit{roscore} a~\EVbrick{\it em} může probíhat přes externí USB WiFi nebo LAN~modul.

Díky ROSu lze s~EV3 pracovat odlišným způsobem. 
Místo, aby se vytvářeli program přímo do \brick{\it u}, připraví se jen jednotlivé topic, přes která se následně získávají data a~posílají příkazy. 
...
Při využívání ROSu společně se stavebnicí \lego{}, je ovšem problematická nutnost mít k~\brick{\it u} připojené klasické PC, na~kterém poběží \textit{roscore}. 
...
% Při portování HRP2 na EV3 bylo nejdůležitější přidání podpory pro MMU\footnote{MMU = Memory Management Unit}
...
Hlavní příčinou je samotný RTOS, který ma velmi nízkou režii na svůj chod a umožňuje rychlý přístup k~hardwaru. % TODO: + rychlejší zpracování operaci a přepínání tasku
